...
class Shaxs:
    ...
    def __lt__(self,y):
        return (2021-self.tyil)<(2021 - y.tyil)
...
class oila:
    ...
    def add_shaxs(self, *people):
        for shaxs in people:
            if isinstance(shaxs, Shaxs):
                self.odamlar.append(shaxs)
            else:
                print("Shaxs kiriting")
                
    def __repr__(self):
        return f"{self.nomi}"
    
    def __len__(self):
        return len(self.odamlar)
                
    def __getitem__(self, index):
        return self.odamlar[index]
    
    def __setitem__(self, index, qiymat):
        self.odamlar[index] = qiymat
        
    def __call__(self, *qiymat):
        if qiymat:
            for shaxs in qiymat:
                self.add_shaxs(shaxs)
        else:
            return [shaxs for shaxs in self.odamlar]
    
    def __add__(self, y):
        if isinstance(y, oila):
            yangi_oila = oila(f"{self.nomi} va {y.nomi}")
            yangi_oila.odamlar = self.odamlar + y.odamlar
            return yangi_oila
        elif isinstance(y, Shaxs):
            self.add_shaxs(y)
        
        else:
            print(f"Oilaga {type(y)} qo'shib bo'lmaydi")
...
class Talaba(Shaxs):
    ...
    def __repr__(self):
        return f"{self.idraqam} {self.bosqich}"
...
talaba2 = Talaba("Xoldarov", "Paxlavon", "FA2994732", 2000, "IO754983")

oila1 = oila("Iskandarovlar oilasi")
oila1.add_shaxs(admin1, user1, talaba1)
oila2 = oila("Nazarovlar oilasi")
oila2.add_shaxs(user1, prof1, talaba2)
